...
- There are roughly 20 frets (metal bars across the neck of the guitar) used to locate notes along the string.
    - Note: This diagram is not to scale. The frets are [spaced _logarithmically_](../patterns/log-fret-spacing.html) to produce the correct pitches.
...
- Taken together, the frets and strings form a grid. Let's define a **grid step** as moving to the next fret or string, as pictured in the diagram above.
    - This is analagous to [City Blocks](./city-blocks.html) in that we treat the two directions on equal footing, even though physically these steps are not the same size.
...
Here, we get a diagonal [gradient](../patterns/gradient.html) from the low E string, increasing slowly both down the string and across the strings.


## Pitch Symmetries

Studying the diagrams from the previous diagram,
we can identify some [symmetries](../patterns/symmetry.html). First, let's look at the absolute pitches again:
...
A different way of thinking about the fretboard is as a [distance function](../patterns/distance-function.html) that measures semitones
...
get a [gradient](../patterns/gradient.html) stretching diagonally across the fretboard.
...
- A repeating pattern of [stripes](../patterns/stripes.html)
- A [plane wave](../patterns/plane-wave.html) with (linear) wavevector of `(1, 5)` semitones/step
- A [wallpaper pattern](../patterns/wallpaper-pattern.html) with `o` symmetry (orbifold notation) generated by translations (5, -1) and (2, 2).

## Scales on a Fretboard

To make a scale on a fretboard, we can draw a contiguous box around 12 distinct pitches to form a [chromatic scale](../patterns/uniform-scale.html) Given that there are many copies of each note available, there are several ways to do this. Let's define some constraints to narrow our search:
...
- The [circle of fourths](../patterns/uniform-scale.html) progression jumps around the bass fretboard a lot
